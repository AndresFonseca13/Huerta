...
const Home = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const { items, totalPages, totalRecords } = await getCocktails(currentPage, pageSize);
                setCocktails(items);
    ...
    return (
        ...
        >
          <div className="text-sm text-gray-600">
            Mostrando {cocktails.length} de {totalRecords} cocteles (Página {currentPage} de {totalPages})
          </div>
          <div className="flex justify-center space-x-2 overflow-x-auto">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
              <motion.button
                ...
                className={`px-3 py-1 rounded transition-colors ${page === currentPage ? 'bg-green-700 text-white' : ''}`}
              >
                {page}
              </motion.button>
            ))}
