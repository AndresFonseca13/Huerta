const validateQueryGetAll = (req, res, next) => {
  ...
  if (orden && !['name', 'price'].includes(orden)) {
    return res
      .status(400)
      .json({ mensaje: 'El parámetro orden solo puede ser "name" o "price"' });
  }

  if (categoria && typeof categoria !== 'string') {
    return res
      .status(400)
      .json({ mensaje: 'El parámetro categoria debe ser un texto' });
  }

  if (tipo && typeof tipo !== 'string') {
    return res
      .status(400)
      .json({ mensaje: 'El parámetro tipo debe ser un texto' });
  }
